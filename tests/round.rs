...
use bung::config::{DefaultConfig, SerializerConfig};
use bung::decode::ReadReader;
use bung::{Deserializer, Serializer};
...
fn issue_250() {
	...
	let v = bung::to_vec(&Example::HasValue {
		x: 3,
	})
	.unwrap();
	...
	let ex: Example = bung::from_slice(&v).unwrap();
	assert_eq!(
		Example::HasValue {
			x: 3
		},
	...
	let v = bung::to_vec(&Example::Unit1).unwrap();
...
fn round_struct_like_enum_with_struct_map() {
	...
	let mut buf = Vec::new();
	expected.serialize(&mut Serializer::new(&mut buf).with_struct_map()).unwrap();
...
fn round_struct_like_enum_with_struct_tuple() {
	...
	let mut buf = Vec::new();
	expected.serialize(&mut Serializer::new(&mut buf).with_struct_tuple()).unwrap();
...
fn round_trip_untagged_enum_with_enum_associated_data() {
	...
	let bytes_1 = bung::to_vec(&data1_1).unwrap();
	let data1_2 = bung::from_slice(&bytes_1).unwrap();
	assert_eq!(data1_1, data1_2);

	let data2_1 = Foo::A(Bar::C("Hello".into()));
	let bytes_2 = bung::to_vec(&data2_1).unwrap();
	let data2_2 = bung::from_slice(&bytes_2).unwrap();
	assert_eq!(data2_1, data2_2);

	let data3_1 = Foo::A(Bar::D(1, 2, 3));
	let bytes_3 = bung::to_vec(&data3_1).unwrap();
	let data3_2 = bung::from_slice(&bytes_3).unwrap();
	assert_eq!(data3_1, data3_2);

	let data4_1 = Foo::A(Bar::E {
		f1: "Hello".into(),
	});
	let bytes_4 = bung::to_vec(&data4_1).unwrap();
	let data4_2 = bung::from_slice(&bytes_4).unwrap();
...
fn round_struct_as_map() {
	use bung::decode::from_slice;
	use bung::to_vec_named;
...
fn round_trip_unit_struct() {
	...
	// struct-as-tuple
	{
		let serialized: Vec<u8> = bung::to_vec(&msg2).unwrap();
		let deserialized: Messages = bung::from_slice(&serialized).unwrap();
		assert_eq!(deserialized, msg2);
	}

	// struct-as-map
	{
		let serialized: Vec<u8> = bung::to_vec_named(&msg2).unwrap();
		let deserialized: Messages = bung::from_slice(&serialized).unwrap();
		assert_eq!(deserialized, msg2);
	}
}
...
fn round_trip_unit_struct_untagged_enum() {
	...
	// struct-as-tuple
	{
		let serialized: Vec<u8> = bung::to_vec(&msga).unwrap();
		let deserialized: Messages = bung::from_slice(&serialized).unwrap();
	...
	// struct-as-map
	{
		let serialized: Vec<u8> = bung::to_vec_named(&msga).unwrap();
		let deserialized: Messages = bung::from_slice(&serialized).unwrap();
		assert_eq!(deserialized, msga);
	}
}
...
fn round_trip_struct_with_flattened_map_field() {
	...
	let serialized: Vec<u8> = bung::to_vec(&strct).unwrap();
	let deserialized: Struct = bung::from_slice(&serialized).unwrap();
...
fn round_trip_struct_with_flattened_struct_field() {
	...
	// struct-as-map
	{
		let serialized: Vec<u8> = bung::to_vec_named(&strct).unwrap();
		let deserialized: Struct = bung::from_slice(&serialized).unwrap();
		assert_eq!(deserialized, strct);
	}
}
...
fn checked_seq_access_len() {
	...
	let mut deserializer = bung::Deserializer::new(Cursor::new(&buffer)).with_binary();
