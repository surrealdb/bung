...
use bung::decode::{self, Error};
use bung::{Deserializer, Raw, RawRef};
use rmp::Marker;
...
fn pass_raw_valid_utf8() {
	...
	let raw: Raw = bung::from_slice(&buf[..]).unwrap();
...
fn pass_raw_ref_valid_utf8() {
	let buf = vec![0xa3, 0x6b, 0x65, 0x79];
	let raw: RawRef<'_> = bung::from_slice(&buf[..]).unwrap();
...
fn fail_str_invalid_utf8() {
	let buf = vec![0xa4, 0x92, 0xcc, 0xc8, 0x90];
	let err: Result<String, decode::Error> = bung::from_slice(&buf[..]);
...
fn fail_depth_limit() {
	...
	let mut reader = bung::Deserializer::new(Cursor::new(data));
